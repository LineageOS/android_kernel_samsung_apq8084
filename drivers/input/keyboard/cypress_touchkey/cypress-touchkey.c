...
static void cypress_touchkey_interrupt_set_dual(struct i2c_client *client)
{
	...
	if (info->device_ver != 0x10) {
		dev_err(&client->dev, "%s: not support this IC %x\n",
			__func__, info->device_ver);
		return;
	}

	if (info->ic_fw_ver < CYPRESS_RECENT_BACK_REPORT_FW_VER) {
		dev_err(&client->dev, "%s: not support this version 0x%02x\n",
...
static int tkey_i2c_check(struct cypress_touchkey_info *info)
{
	...
	dev_info(&client->dev,
		"%s: ic_fw_ver = 0x%02x, module_ver = 0x%02x,"
		"CY device = 0x%02x, system_rev:%d\n",
		__func__, info->ic_fw_ver, info->module_ver,
		info->device_ver, system_rev);

	/* CYPRESS Firmware setting interrupt type : dual or single interrupt */
	cypress_touchkey_interrupt_set_dual(client);
...
#else
		dev_info(&info->client->dev,
				"%s: key %s. fw_ver=0x%x, module_ver=0x%x \n",
				__func__, press ? "pressed" : "released",
				info->ic_fw_ver, info->module_ver);
#endif
		if (code < 0) {
			dev_info(&info->client->dev,
...
static int load_fw_built_in(struct cypress_touchkey_info *info)
{
	...
	if(info->pdata->ldo_flag) {
		fw_name = kasprintf(GFP_KERNEL, "%s/%s.fw",
			TKEY_FW_BUILTIN_PATH, TKEY_SEMCO_L_CYPRESS_FW_NAME);
	}

	/* SEMCO module ver: 0x10 */
	else {
		fw_name = kasprintf(GFP_KERNEL, "%s/%s.fw",
			TKEY_FW_BUILTIN_PATH, TKEY_SEMCO_CYPRESS_FW_NAME);
	}

	pr_info("tkey firmware name = %s\n",fw_name);
...
static int load_fw_in_sdcard(struct cypress_touchkey_info *info)
{
	...
	if(info->pdata->ldo_flag) {
		fw_name = kasprintf(GFP_KERNEL, "%s/%s.fw",
			TKEY_FW_IN_SDCARD_PATH, TKEY_SEMCO_L_CYPRESS_FW_NAME);
	}
			...
			struct cypress_touchkey_platform_data *pdata)
{
	...
	/* regulator info */
	pdata->ldo_flag = of_property_read_bool(np, "cypress,led-ldo");
	...
	pr_err("%s: tkey_led = %d, tkey_scl= %d, tkey_sda= %d, tkey_int= %d",
			__func__, pdata->ldo_flag, pdata->gpio_scl, pdata->gpio_sda, pdata->gpio_int);
...
*/
	...
	if (ret < 0) {
		...
		#if defined(CONFIG_SEC_FACTORY) && !defined(CONFIG_SEC_KCCAT6_PROJECT)
			info->enabled = false;
			goto err_i2c_check;
		#else
			bforced = true;
...
#ifdef CONFIG_SEC_TRLTE_PROJECT  // jjlee_temp
	{
		int update_pass = 1;
		if(!update_pass){
#endif
