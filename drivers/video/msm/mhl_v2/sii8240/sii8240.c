...
static void sii8240_link_monitor_work(struct work_struct *work)
{
	...
	if (ret < 0)
		...
		pr_info("sii8240: %s(): PAGE_2:0xA0:0x%02x !\n", __func__, rd_data2);
...
static int sii8240_hdcp_on(struct sii8240_data *sii8240, bool hdcp_on)
{
	...
	if (hdcp_on) {
		...
		if (sii8240->pdata->ddcline_connected)
...
{
	struct sii8240_data *sii8240 = dev_get_drvdata(sii8240_mhldev);
	uint8_t version = sii8240->regs.peer_devcap[MHL_DEVCAP_MHL_VERSION];

	return version < CONFIG_MHL_VERSION ? version : CONFIG_MHL_VERSION;
}

int mhl_get_video_linkmode(void)
{
	struct sii8240_data *sii8240 = dev_get_drvdata(sii8240_mhldev);
	return sii8240->regs.peer_devcap[MHL_DEVCAP_VID_LINK_MODE];
}

u8 *mhl_get_edid(void)
{
		...
		unsigned long event, void *ptr)
{
	...
	if (!sii8240->pdata->bootup_complete) {
		pr_info("sii8240 : platform is not ready yet\n");
		return NOTIFY_DONE;
	}
...
static void sii8240_detection_bootup(struct work_struct *work)
{
	struct sii8240_platform_data *sii8240 =
		container_of(work, struct sii8240_platform_data, bootup_work.work);
	struct sec_mhl_cable *cable;
	int i, ret = 0;

	pr_info("sii8240: %s +++\n", __func__);
	sii8240->bootup_complete = true;

	for (i = 0; i < ARRAY_SIZE(support_cable_list); i++) {
		cable = &support_cable_list[i];
		if (cable->cable_type != EXTCON_MHL_VB && cable->edev != NULL) {
			ret = extcon_get_cable_state(cable->edev,
						extcon_cable_name[cable->cable_type]);
			if (ret) {
				pr_info("sii8240: %s:%s - ret:%d\n", __func__,
					extcon_cable_name[cable->cable_type], ret);
				sii8240_extcon_notifier(&(cable->nb), ret, NULL);
			}
		}
	}
}
			...
			const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&sii8240->pdata->bootup_work, sii8240_detection_bootup);
	schedule_delayed_work(&sii8240->pdata->bootup_work, msecs_to_jiffies(2000));
#endif
#ifdef SII8240_CHECK_MONITOR
