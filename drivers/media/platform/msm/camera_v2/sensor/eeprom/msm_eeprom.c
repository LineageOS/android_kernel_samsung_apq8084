...
static int msm_eeprom_verify_sum(const char *mem, uint32_t size, uint32_t sum)
{
	...
	if (~crc != sum) {
		CDBG("%s:ERROR expect 0x%x, result 0x%x CHK INTF-EEROM\n", __func__, sum, ~crc);
		return -EINVAL;
	}
...
static uint32_t msm_eeprom_match_crc(struct msm_eeprom_memory_block_t *data)
{
	...
	map_ver = data->mapdata[MAP_VERSION_ADDR];
	pr_err("%s: map version = %c \n", __func__, map_ver);
	for (j = 0; j + 1 < data->num_map; j += 2) {
		...
		//  There is 496 (=1040-544) AF data more at v005 than v004 starting from 0x5000.
		if(map_ver == '4' && j == 8)  //  address 0x5000 at v004.
		{
			rc = msm_eeprom_verify_sum(memptr, map[j].mem.valid_size-496, *sum);
			pr_err("%s: if map_ver==4, num of AF data is %d, not %d \n",
				__func__,
				map[j].mem.valid_size-496, map[j].mem.valid_size);
		} else {
...
static int msm_eeprom_match_id(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	pr_info("%s: read 0x%02X%02X, check Fidelix 32Mb:0xF815, Winbond 32Mb:0xEF15, Macronix 32Mb:0xC236, GigaDevice 32Mb:0xC815\n",
		__func__, id[0], id[1]);
	...
	struct msm_eeprom_cfg_data *cdata)
{
   ...
   static uint8_t isFWcheck = 0;
	...
	pr_err("%s: address (0x%x) comp_size (%d) after decomp (%d) \n", __func__,
  ...
  if (rc < 0) {
    ...
    if (isFWcheck == 0) {
      msm_camera_fw_check('F', 0);//F: Fail
      isFWcheck++;
    }
    goto POWER_DOWN;
  }
