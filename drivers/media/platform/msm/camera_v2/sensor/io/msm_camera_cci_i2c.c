	...
	enum msm_camera_i2c_data_type data_type)
{
	...
	if (rc < 0) {
		pr_err("%s: line %d i2c read fail[Slave Addr::0x%x] rc = %d\n", __func__, __LINE__, client->cci_client->sid, rc);
		return rc;
	}
	rc = cci_ctrl.status;
	...
	uint32_t addr, uint8_t *data, uint32_t num_byte)
{
	...
	int i;
	struct msm_camera_cci_ctrl cci_ctrl;
	pr_err("%s before addr = 0x%x data: 0x%x\n", __func__, addr, *data);
	...
	enum msm_camera_i2c_data_type data_type)
{
	...
	rc = v4l2_subdev_call(client->cci_client->cci_subdev,
			core, ioctl, VIDIOC_MSM_CCI_CFG, &cci_ctrl);
	if (rc < 0) {
		pr_err("%s: line %d i2c write fail[Slave Addr::0x%x] rc = %d\n", __func__, __LINE__, client->cci_client->sid, rc);
		return rc;
	}
	rc = cci_ctrl.status;
	return rc;
	...
	struct msm_camera_i2c_reg_setting *write_setting)
{
	...
	CDBG("[syscamera:IIC][%s][E]", __FUNCTION__);

	if (!client || !write_setting) {
		pr_err("[syscamera:IIC][%s:%d][Error]", __FUNCTION__, __LINE__);
		return rc;
	}
	...
	if (write_setting->data_type == MSM_CAMERA_I2C_BURST_DATA) {
		...
		burst_reg_array = (struct msm_camera_i2c_burst_reg_array *)write_setting->reg_setting;

		CDBG("[syscamera:IIC][%s][BURST_WRITE]", __FUNCTION__);
	...
	else
	{
		...
		CDBG("[syscamera:IIC][%s][write_setting->size::%d]", __FUNCTION__, write_setting->size);

	for (i = 0; i < write_setting->size; i++) {
		S_I2C_DBG("[syscamera:IIC][idx:%04d] 0x%04x 0x%04x 0x%02x", i, reg_setting->reg_addr, reg_setting->reg_data, reg_setting->data_type);
#ifndef BYPASS_COMPANION
		rc = msm_camera_cci_i2c_write(client, reg_setting->reg_addr,
					      reg_setting->reg_data, reg_setting->data_type);
#else
		rc = msm_camera_cci_i2c_write(client, reg_setting->reg_addr,
