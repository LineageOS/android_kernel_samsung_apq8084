...
static int max86900_regulator_onoff(struct max86900_device_data *data, int onoff)
{
	...
	if (onoff == HRM_LDO_ON) {
		...
		rc = regulator_enable(data->vdd_1p8);
		if (rc) {
			pr_err("enable vdd_1p8 failed, rc=%d\n", rc);
			return -ENODEV;
		}
	} else {
		rc = regulator_disable(data->vdd_1p8);
		if (rc) {
			pr_err("disable vdd_1p8 failed, rc=%d\n", rc);
			return -ENODEV;
		}
	}
...
static ssize_t max86900_uv_poll_delay_store(struct device *dev,
					...
					const char *buf, size_t size)
{
	pr_info("%s - max86900 uv sensor delay was fixed as 1000ms\n", __func__);
	return size;
}
...
#if 0
	if (of_property_read_string(dNode, "max86900,sub_ldo4",
		&data->sub_ldo4) < 0)
...
//	data->default_current = MAX86900C_DEFAULT_CURRENT;
	...
	err = sensors_create_symlink(&data->uv_input_dev->dev.kobj,
...
err_uv_sysfs_create_group:
	sensors_remove_symlink(&data->uv_input_dev->dev.kobj,
			data->uv_input_dev->name);
