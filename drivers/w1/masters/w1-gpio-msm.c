...
#if defined(CONFIG_W1_SLAVE_DS28EL35) /* DS28EL35 only(for IRQ mode) */
...
static void w1_gpio_write_8(void *data, u8 byte)
{
	...
	spin_lock_irqsave(&w1_gpio_msm_lock, irq_flags);
	for (i = 0; i < 8; ++i)
		w1_gpio_write_bit_in8(data, (byte >> i) & 0x1);
...
static u8 w1_gpio_read_bit(void *data)
{
	...
	(pdata->slave_speed == 0)? w1_delay(55) : w1_delay(10);
