...
#ifdef GSCAN_SUPPORT
		else if (band == WLC_BAND_5G_DFS_ONLY) {
			if (skip_dfs || !is_dfs(dtoh32(list->element[i]))) {
				continue;
			}
		}
...
dhd_pno_get_legacy_pno_ssid(dhd_pub_t *dhd, dhd_pno_status_info_t *pno_state)
{
	...
	list_for_each_entry_safe(iter, next, &_params1->params_legacy.ssid_list, list) {
		if (i >= _params1->params_legacy.nssid) {
			break;
		}
	...
	void *info, uint32 *len)
{
	...
	switch (type) {
		...
		default:
			DHD_ERROR(("%s: out of PNO gscan cmd type(%d)\n", __FUNCTION__, type));
			break;
	}
	...
	void *buf, uint8 flush)
{
	...
	switch (type) {
		...
		case DHD_PNO_GEOFENCE_SCAN_CFG_ID:
			{
				gscan_hotlist_scan_params_t *ptr =
					(gscan_hotlist_scan_params_t *)buf;
				...
				if ((_params->params_gscan.nbssid_hotlist +
					...
					DHD_ERROR(("Excessive number of hotlist APs programmed "
				...
				for (i = 0, bssid_ptr = ptr->bssid; i < ptr->nbssid; i++,
					bssid_ptr++) {
					_pno_bssid = kzalloc(sizeof(struct dhd_pno_bssid),
						GFP_KERNEL);

					if (!_pno_bssid) {
						DHD_ERROR(("_pno_bssid is NULL, cannot kalloc "
							"%zd bytes",
							sizeof(struct dhd_pno_bssid)));
						err = BCME_NOMEM;
						_params->params_gscan.nbssid_hotlist += i;
						dhd_pno_reset_cfg_gscan(_params, _pno_state,
							GSCAN_FLUSH_HOTLIST_CFG);
						goto exit;
					}
					memcpy(&_pno_bssid->macaddr, &bssid_ptr->macaddr,
						ETHER_ADDR_LEN);
		...
		case DHD_PNO_SIGNIFICANT_SCAN_CFG_ID:
			{
				...
				for (i = 0, significant_bssid_ptr = ptr->bssid_elem_list;
					...
					if (!_pno_significant_change_bssid) {
						DHD_ERROR(("SWC bssidptr is NULL, cannot kalloc "
							"%zd bytes",
		...
		case DHD_PNO_SCAN_CFG_ID:
			{
				...
				if (ptr->nchannel_buckets <= GSCAN_MAX_CH_BUCKETS) {
					...
					memcpy(_params->params_gscan.channel_bucket,
						ptr->channel_bucket,
						_params->params_gscan.nchannel_buckets *
						sizeof(struct dhd_pno_gscan_channel_bucket));
					...
					for (i = 0; i < ptr->nchannel_buckets; i++) {
						...
						for (k = 0; k < ptr->channel_bucket[i].num_channels;
							k++) {
							ch_bucket[i].chan_list[k] =
								wf_mhz2channel(
								ptr->channel_bucket[i].chan_list[k],
								0);
						}
						ch_bucket[i].band = 0;
						/* HAL and DHD use different bits for 2.4G and
...
dhd_pno_set_for_gscan(dhd_pub_t *dhd, struct dhd_pno_gscan_params *gscan_params)
{
	...
	if (gscan_params->nbssid_significant_change) {
		...
		nbssid_significant_change = gscan_params->nbssid_significant_change;
	...
	uint32 *num_buckets_to_fw)
{
	...
	*num_buckets_to_fw = *num_buckets;
	...
	mem_needed = ((*num_buckets) * sizeof(wl_pfn_gscan_channel_bucket_t));
	ch_bucket = (wl_pfn_gscan_channel_bucket_t *) MALLOC(dhd->osh, mem_needed);
...
int
dhd_gscan_batch_cache_cleanup(dhd_pub_t *dhd)
{
	int ret = 0;
	dhd_pno_params_t *params;
	struct dhd_pno_gscan_params *gscan_params;
	...
	while (iter && (iter->tot_consumed == iter->tot_count)) {
			tmp = iter->next;
			kfree(iter);
			iter = tmp;
	}
			      ...
			      int *size)
{
	...
	*size = 0;
	...
	bssinfo_length = (dtoh32(gscan_result->buflen) -
		WL_GSCAN_RESULTS_FIXED_SIZE -
		WL_GSCAN_INFO_FIXED_FIELD_SIZE);
	if (bi_length != bssinfo_length) {
	...
	*send_evt_bytes = 0;
	...
	for (i = 0; i < results->count; i++, plnetinfo++) {
		...
		DHD_PNO(("\t%s "MACDBG" rssi %d\n", hotlist_found_array->ssid,
			MAC2STRDBG((u8*)&hotlist_found_array->macaddr),
