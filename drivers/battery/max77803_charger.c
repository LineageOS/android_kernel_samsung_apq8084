...
static int max77803_charger_probe(struct platform_device *pdev)
{
	...
	ret = request_threaded_irq(charger->irq_bypass, NULL,
			max77803_bypass_irq, IRQF_ONESHOT, "bypass-irq", charger);
	if (ret < 0)
...
static int max77803_charger_remove(struct platform_device *pdev)
{
	struct max77803_charger_data *charger =
...
static struct platform_driver max77803_charger_driver = {
	...
	.remove = max77803_charger_remove,
};
